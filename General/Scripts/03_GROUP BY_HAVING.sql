...
SELECT DEPT_CODE , COUNT(DEPT_CODE) -- 3번째로 해석 
FROM EMPLOYEE 			-- 1번째로 해석 
GROUP BY DEPT_CODE;	-- 2번째로 해석 
...
SELECT 
	DEPT_CODE , FLOOR(AVG(SALARY)) "평균 급여" 
FROM
	EMPLOYEE
WHERE 
	DEPT_CODE IN('D5', 'D6')
...
-- 부서별 평균가 급여 3500000원 이상인 부서를 조회하여 
-- 부서코드 오름차순으로 정렬
