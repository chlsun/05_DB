...
-- 5번째 문자 검색 시작, 3번째로 찾은 C의 위치 조회
SELECT 'AABBCCABC', INSTR('AABBCCABC', 'A', 1, 1)
FROM DUAL;
...
SELECT 
	CEIL(MONTHS_BETWEEN(
		TO_DATE('2025-07-17', 'YYYY-MM-DD'), CURRENT_DATE)) || '개월'
FROM DUAL;
...
SELECT 
	...
	FLOOR(
		MONTHS_BETWEEN(
			SYSDATE, TO_DATE('2001.03.20', 'YYYY.MM.DD')) / 12
	) AS "만 나이"
FROM DUAL;
...
/* (참고)
 * - 문자열 정렬 기준은 글자수, 글자 순서 등에 영향이 있기 떄문에
 * 	 정렬 시 생각을 자리 해봐야함
 * 
...
FROM EMPLOYEE 
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) -- 서브 쿼리
...
/* 방법 2 - COUNT(컬럼명) 이용 */
SELECT COUNT(PHONE)	AS PHONE -- PHONE 컬럼의 값이 NULL인 경우 제외하고 카운트
FROM EMPLOYEE;
...
SELECT 
	COUNT(
		DECODE(
			SUBSTR(EMP_NO,8,1), 1, '남자'
	)) AS 남자,
	COUNT(
		DECODE(
			SUBSTR(EMP_NO,8,1), 2, '여자'	
	)) AS 여자
FROM EMPLOYEE ;
