...
-- EMPLOYEE 테이블에서
-- 모든 사원의 이름, 급여, 급여 + 100만원
SELECT EMP_NAME, SALARY, SALARY + 1000000
FROM EMPLOYEE;


-- 모든 사원의 이름, 급여(1개원), 연복(급여 * 12) 조회
...
SELECT 
	...
	CEIL((CURRENT_DATE - HIRE_DATE) / 365) AS "연차 조회" -- 올림 처리
FROM EMPLOYEE;
 ...
 * 
 *  * SELECT절에 리터럴을 작성하면
 * 		조회 결과(RESULT SET) 모든 행에 리터럴이 추가 된다!!  
 */
...
/* *** NULL 비교 연산 ***
 * 
 * 1) 컬러명 IS NULL : 해당 컬럼 값이 NULL 이면 TRUE 반환
 * 
 * 2) 컬럼명 IS NOT NULL : 해당 컬럼 값이 NULL이 아니면 TRUE반환
 * 												== 컬럼에 값이 존재하면 TRUE
 * 
 ...
 * 
 * - OR(또는)
 * 	두 연산의 결과가 을 모두FALSE 일때만 최종 결과 FALSE
 * 	-> 두 조건 중 하나라도 만족하는 행을 
 * 		 결과 집합(RESULT SET)에 포함
 * 
 * - 우선 순위 : AND > OR
...
SELECT EMP_ID , EMP_NAME , SALARY 
FROM EMPLOYEE 
WHERE SALARY >= 3000000 AND SALARY < 5000000; -- 16행 조회
 ...
 * 
 * - 컬럼 값이 (A) 이상 (B) 이하인 경우 FALSE(조회 하겠다)
 * 	-> (A) 미만, (B) 초과인 경우 TRUE
 */
...
WHERE 
	...
	AND 
	TO_DATE('2019.12.31', 'YYYY.MM.DD'); -- 행
 ...
 * 1) '%' (포함)
 * 
 * 	'%A' : 'A'로 끝나는 문자열인 경우 TRUE
 * 				-> 앞쪽에는 어떤 문자열이든 관계 없음(빈 칸도 가능)
 * 
 * 	'A%' : A로 시작하는 문자열인 경우 TRUE
 * 
 * 	'%A%' : A를 포함하는 문자열인 경우 TRUE (맨앞, 맨뒤에 존재해도 TRUE)
 * 
 * 2) '_' (글자 수, _ 1개당 1글자)
 * 
 * ___ : 문자열이 3글자이 경우 TRUE
 * 
 ...
 * 
 * - 작성법
 *  WHERE LIKE '___#_' ESCAPE '#' 		('#'말고 다른 특수문자 다 됨)
 * -> '#' 바로 뒤 '_'는
 * 	와일드 카드 x, 단순 문자열 O
 */
SELECT EMP_NAME , EMAIL 
