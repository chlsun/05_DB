...
-- 21행 조회
-- 왜? 23행이 아니라 21행??
-- EMPLOYEE에 DEPT_CODE 값이 NULL인 행 2개 있지만
...
-- 사원의 급여가
-- SAL_LAVEL에 작성된 최소(MIN_SAL) ~ 최대(MAX_SAL)
...
SELECT 
	...
	NVL(MGR.EMP_ID, '없음') "사수 번호", NVL(MGR.EMP_NAME, '없음') "사수 이름"
FROM 
	EMPLOYEE EMP
LEFT JOIN
...
-- 1) ON 이용 방법 -> JOIN에 사용된 컬럼이 1개로 합쳐짐
...
SELECT 
	EMP_NAME , E.EMP_NO , D.DEPT_TITLE , J.JOB_NAME 
FROM 
	EMPLOYEE E
JOIN 
...
SELECT 
	...
	E.SALARY * (NVL(BONUS, 0) + 1) * 12 AS 연봉
FROM 
	EMPLOYEE E
JOIN 
...
SELECT 
	E.EMP_NAME 사원명, D.DEPT_TITLE 부서명, L.LOCAL_NAME 지역명,
	DECODE(L.NATIONAL_CODE , 'KO', '한국', 'JP', '일본') 국가명
FROM 
	EMPLOYEE E
JOIN 
	DEPARTMENT  D ON (D.DEPT_ID = E.DEPT_CODE)
...
-- SELF JOIN 사용
SELECT 
	E.EMP_NAME 사원명, E.DEPT_CODE 부서코드, E2.EMP_NAME 동료이름
FROM 
	EMPLOYEE E
JOIN 
	EMPLOYEE E2 ON (E.DEPT_CODE = E2.DEPT_CODE 
