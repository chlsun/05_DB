...
SELECT * FROM V_DCOPY2;	-- DEPT_ID, DEPT_TITLE 만 보임

-- V_DCOPY2에 다시 INSERT
INSERT INTO V_DCOPY2
VALUES('A1', '마케팅1팀');
-- NULL을 ("KH26_CYS"."DEPT_COPY2"."LOCATION_ID") 안에 삽입할 수 없습니다
...
/* 인덱스를 활용해서 검색하는 방법!
 * - SELECT문의 WHERE절에 INDEX가 설정된 컬럼을 언급하면된다
 */
 ...
 */ 
SELECT * FROM TB_IDX_TEST 
WHERE TEST_ID = 'TEST500000'; -- 0.035s

/* 인덱스를 이용하지 않고 조회하기
 * - TEST_NO 컬럼 값이 500000인 행 조회
 */ 
SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO = 500000; -- 0.027s
