 ...
 * - GRANT 권한,.... TO 사용자명;
 */
GRANT CREATE SESSION TO TEST26_USER;
--> 연결 확인됨


/* 생성된 사용자 계정으로 접속 (TEST26_USER) */
CREATE TABLE TEST_TABLE(
	TEST_NO NUMBER PRIMARY KEY,
	TEST_COL VARCHAR2(30) NOT NULL
);
--> SQL Error 권한이 불충분합니다
	-- + 사용 가능한 공간(TABLESPACE) 할당


/* 다시 관리자로 접속 */
GRANT RESOURCE TO TEST26_USER;
...
/* TEST26_USER에게 테이블 생성 공간 할당 
 * - USERS폴더 내에 10M 할당
 * */
ALTER USER TEST26_USER
DEFAULT TABLESPACE USERS
QUOTA 10M ON USERS;


/* 다시 TEST26_USER 계정 계쩡 접속 후 테이블 생성 */
CREATE TABLE TEST_TABLE(
	TEST_NO NUMBER PRIMARY KEY,
	TEST_COL VARCHAR2(30) NOT NULL
);
