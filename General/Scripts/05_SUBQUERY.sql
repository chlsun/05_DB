...
SELECT 
	E.EMP_NAME, J.JOB_CODE, D.DEPT_TITLE, E.SALARY 
FROM 
	EMPLOYEE E
JOIN
...
SELECT 
	E.EMP_ID, E.EMP_NAME,
	NVL(D.DEPT_TITLE, '없음') AS DEPT_TITLE, J.JOB_NAME,
	CASE
		-- 현재 행의 EMP_ID와 일치하는 값이 
		-- 서브쿼리 결과에 있으면 '사수', 없으면 '사원'
		WHEN E.EMP_ID IN (
			SELECT DISTINCT MANAGER_ID -- 다중행 서브쿼리
			FROM EMPLOYEE
...
WHERE 	
	J.JOB_NAME = '대리'
AND 
	E.SALARY > (	-- 과자 최소 급여(320만)보다 많이 받는 대리 조회
		SELECT MIN(E.SALARY) 
		FROM EMPLOYEE E
...
AND 
	E.SALARY > ALL (	
		SELECT E.SALARY -- 380만, 348만, 349만, 255만
		FROM EMPLOYEE E
		JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
		WHERE J.JOB_NAME = '차장'
);              
...
FROM EMPLOYEE 
WHERE DEPT_CODE IN( 
	SELECT DEPT_ID	-- 2) 서브쿼리
	FROM DEPARTMENT 
	WHERE LOCATION_ID = ( 
		SELECT LOCAL_CODE	-- 1) 서브쿼리
		FROM LOCATION 
		WHERE NATIONAL_CODE ='KO'
	)
);
...
WHERE 
	MAIN.HIRE_DATE = (
		...
		FROM
			EMPLOYEE SUB 
		WHERE 
			NVL(SUB.DEPT_CODE, '소속 없음') = NVL(MAIN.DEPT_CODE, '소속 없음')
		AND
			ENT_YN != 'Y'
	)
ORDER BY 
...
-- (스칼라 + 상관 쿼리)
SELECT MAIN.EMP_ID ,MAIN.EMP_NAME , NVL(MAIN.MANAGER_ID, '없음') AS "관리자사번" , 
	NVL((
		SELECT SUB.EMP_NAME
		FROM EMPLOYEE SUB
...
SELECT MAIN.EMP_ID , MAIN.EMP_NAME , NVL(D.DEPT_TITLE, '소속없음') ,J.JOB_NAME, MAIN.HIRE_DATE 
FROM EMPLOYEE MAIN
LEFT JOIN
	DEPARTMENT D ON (MAIN.DEPT_CODE = D.DEPT_ID)
JOIN 
...
SELECT MAIN.EMP_ID , MAIN.EMP_NAME , J.JOB_NAME , MAIN.EMP_NO,
	FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('19' || SUBSTR(MAIN.EMP_NO, 1, 6), 'YYYYMMDD')) / 12) AS "만 나이",
	TO_CHAR(SALARY * (NVL(MAIN.BONUS, 0) + 1) * 12, 'L999,999,999') AS 연봉
FROM EMPLOYEE MAIN
JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE)
WHERE TO_DATE('19' || SUBSTR(MAIN.EMP_NO, 1, 6), 'YYYYMMDD') = (
	SELECT MAX(TO_DATE('19' || SUBSTR(SUB.EMP_NO, 1, 6), 'YYYYMMDD'))
	FROM EMPLOYEE SUB
	WHERE SUB.JOB_CODE = MAIN.JOB_CODE
)
ORDER BY "만 나이" DESC;
